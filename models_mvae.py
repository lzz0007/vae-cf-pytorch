...
class MultiVAE(nn.Module):
    ...
    def __init__(self, p_dims, q_dims=None, dropout=0.5, tau=0.1, std=0.075, kfac=7, nogb=False):
        ...
        # center for title
        self.cores_title = nn.Parameter(torch.empty(self.kfac, 100*128))
        nn.init.xavier_normal_(self.cores_title.data)
        # for title encoder
        self.fc1_enc = nn.Embedding(17424, 128)
        self.fc2_enc = nn.Linear(128 * 100 * 102, 128)
        self.fc31_enc = nn.Linear(128, 100)
        self.fc32_enc = nn.Linear(128, 100)
        # for title decoder
